...
class KdTree:
    ...
    def points_in_range(self, query_rectangle):
        ...
        if self.is_leaf:
            matches = [point for point in self.points if point.in_box(bottom_left, top_right)]
        else:
            matches = []
            if bottom_left[self.axis] <= self.coord: # rectangle intersects left (or bottom) half space
                matches += self.leftorbottom.points_in_range(query_rectangle)
            if top_right[self.axis] >= self.coord: # rectangle intersects right (or top) half space
