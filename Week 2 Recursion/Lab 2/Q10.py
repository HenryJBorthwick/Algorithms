...
def multiply2by2(A, B):
    ...
    product = [        [A[0][0]*B[0][0]+A[0][1]*B[1][0], A[0][0]*B[0][1]+A[0][1]*B[1][1]],
...
def matrix_power(A, n):
    ...
    # if n is 1 then return the original matrix A.
    if n == 1:
        return A

    # if n is even then use the formula (A^2)^(n/2) = (A^n)^2.
    if n % 2 == 0:
        X = matrix_power(A, n//2)
        return multiply2by2(X, X)

    # if n is odd then use the formula A^(n-1) = A * A^(n-2).
    Y = matrix_power(A, n-1)
    return multiply2by2(A, Y)
